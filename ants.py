...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        # the exit is itself a place - if this Place has an exit then make exit's entrance = this Place
        if exit != None:
            exit.entrance = self
        # END Problem 2
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                # self will be a container in this method
                # if the ant in place is a container and is empty then insert the insect inside container
                if self.ant.container == True and self.ant.ant == None:
                    self.ant.contain_ant(insect)
                # if the insect passed in to add_insect can contain the ant currently in place then do so
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect,QueenAnt):
                ...
                if hasattr(self.ant, 'container') and self.ant.container:
                    ...
                    if hasattr(insect,'isQueen'):
                        if not insect.isQueen:
                            # only remove if insect contained is not a true QueenAnt
                            self.ant.ant = None
                    else:
                        # regular ants inside containers
                        self.ant.ant = None
            else:
        ...
        # dont remove place from insect if it's true QueenAnt
        if not (isinstance(insect,QueenAnt) and insect.isQueen):
            insect.place = None
...
class Insect(object):
    ...
    is_ant = False
    damage = 0
    # Q3 - adding watersafe attribute (False by default)
    watersafe = False
...
class Bee(Insect):
    ...
    name = 'Bee'
    damage = 1
    # Q3 - overide the default of watersafe
    watersafe = True
...
class Ant(Insect):
    ...
    food_cost = 0
    blocks_path = True # from Q8
    container = False # from Q11
...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 3
    # not constrained with min and max range (this applies to Long and ShortThrower)
    min_range = 0
    max_range = 1000000 # large number (math.inf might not be available on test server)
    ...
    def nearest_bee(self, hive):
        ...
        # initialise the location for ranged attacks
        location = 0
        while 1 > 0:
            # first check the current Place of ThrowerAnt for bees
            if random_or_none(checking_place.bees) != None and (self.min_range <= location <= self.max_range):
                return random_or_none(checking_place.bees)
            else:
                checking_place  = checking_place.entrance
                location += 1
            if checking_place == hive:
                return None
        # END Problem 5
...
class WallAnt(Ant):
    """ Ant which does nothing each turn but has a large armor value to stop bees"""
...
class ScubaThrower(ThrowerAnt):
    """ Subclass of ThrowerAnt that is more costly and watersafe """
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 3
        # if insect is watersafe then add it to Water Place using add_insect method in Place
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4
        # utilise the Insect class' reduce_armor method
        current_place = self.place # preserve this as reduce_armor will set it = None when insect dies
        Insect.reduce_armor(self, amount)
        # if insect's armor hits 0 deal damage to all bees in insects place
        if self.armor == 0:
            bees_in_place = current_place.bees  # a list of bees in FireAnt's place of death
            # make a copy of bee_in_place then iterate over and reduce_armor
            bees_to_hurt = list(bees_in_place)
            for bee in bees_to_hurt:
...
class LongThrower(ThrowerAnt):
    ...
    # BEGIN Problem 6
    implemented = True   # Change to True to view in the GUI
    # Modify nearest_bee method to impose minimum range
    food_cost = 2
    armor = 1
    min_range = 5
    # END Problem 6
...
class NinjaAnt(Ant):
    ...
    # BEGIN Problem 8
    implemented = True   # Change to True to view in the GUI
    blocks_path = False   # overrides Ant class attribute
    food_cost = 5
    armor = 1
    # END Problem 8
    ...
    def action(self, colony):
        # BEGIN Problem 8
        # utilise the Insect class' reduce_armor method
        bees_in_place = self.place.bees  # a list of bees in FireAnt's place of death
        # make a copy of bee_in_place then iterate over and reduce_armor
        bees_to_hurt = list(bees_in_place)
        for bee in bees_to_hurt:
            Insect.reduce_armor(bee,self.damage)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 10
        # pythonic implementation - self.place.bees is a list - checking if list is empty
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 11
        # Action without a contained ant should not error
        if self.ant != None:
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 12
        # carries out contained ants action as well as dealing 1 damage to all bees in palce
...
# END Problem 13
    ...
    food_cost = 7
    armor = 1
    queen_count = 0 # keep track of number of number of queens (there should only be one)
    ants_with_double_damage = [] # instantiate an empty list to populate with ants that's damage have been doubled
    # END Problem 13

    def __init__(self):
        # BEGIN Problem 13
        # decide if QueenAnt is an Impostor
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        # deal with impostors
        # check queen is one true queen - if queen is inside container then remove queen from container
        if self.isQueen == False:
            Insect.reduce_armor(self,self.armor)
        elif self.container and self.ant.isQueen == False:
            print('triggered')
            Insect.reduce_armor(self.ant,self.ant.armor)
        else:
            # throw leaf and double damage of all ants in tunnel (which havent been previouslty doubled)
            # throw leaf
            ScubaThrower.action(self, colony)
            # iterate backwards through exits while exist is not None
            current_place = self.place
            while current_place is not None:
                # check if ant has double damage if not double damage of ant and save to list - NB/ dont double damange of queenAnts
                ant = current_place.ant
                # if there is an ant in the current place and the ant is not buffed already
                if (ant != None) and (not hasattr(ant,"isQueen")):
                    # double damage of ant in containers as well as container ant
                    if (ant.container) and (ant.ant != None) and (ant.ant not in QueenAnt.ants_with_double_damage) and (not hasattr(ant.ant,"isQueen")):
                        ant.ant.damage = 2*ant.ant.damage
                        QueenAnt.ants_with_double_damage.append(ant.ant)
                    # check ant hasnt already been doubled
                    if ant not in QueenAnt.ants_with_double_damage:
                        ant.damage = 2*ant.damage
                        QueenAnt.ants_with_double_damage.append(ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.isQueen:
            bees_win()
        # END Problem 13
